...
#: 「付着量記号別の中央値を出すだけ」に対して、モデルがどれだけ上乗せできているか。
#: tools/rolling_validation.py の4フォールド検証（Mar/Apr/May/Jun）で実測した値。
#: 中央値ベースラインは学習設定に依存しないので、配布モデルの検証値と直接比較できる。
MODEL_SKILL: dict[str, dict[str, Any]] = {
    "GI": {
        "median_only_mae": 7.40,
        "model_mae": 6.72,
        "gain_gm2": 0.68,
        "gain_ratio": 0.093,
        "useful": True,
        "note": "モデルは記号別中央値より 9.3% 精度が高く、条件の違いを反映できています。",
    },
    "GL": {
        "median_only_mae": 3.29,
        "model_mae": 3.20,
        "gain_gm2": 0.09,
        "gain_ratio": 0.027,
        "useful": False,
        # 4フォールドで +0.41 / -0.46 / -0.01 / +0.05 と符号が定まらない
        "note": ("モデルの精度は記号別中央値とほぼ同じです（改善 2.7%・月別では符号も定まらない）。"
                 "条件を変えたときの予測の動きは、実績データから裏づけられていません。"),
    },
}

#: 直前のコイルと付着量記号が変わった直後は、誤差が実測でこの倍率になる。
#: tools/error_factors.py（アウトオブフォールド誤差・いずれも p<0.0001）
#:   GI 9.22 / 7.36 = 1.25倍（n=809）   GL 6.95 / 3.14 = 2.21倍（n=153）
#: 条件を記録した時刻と実際にワイピングされた位置の搬送遅れ、および
#: 切替過渡が代表値に混じることが原因と考えられる。
CODE_CHANGE_ERROR_FACTOR: dict[str, float] = {"GI": 1.25, "GL": 2.21}
